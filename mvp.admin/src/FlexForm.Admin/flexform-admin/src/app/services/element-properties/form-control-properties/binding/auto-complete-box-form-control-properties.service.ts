...
import { FieldDefinitionsService } from '../../../../services/low-level-properties/field-definitions.service';
...
export class AutoCompleteBoxFormControlPropertiesService extends FormControlBindingPropertiesService {
  ...
  constructor(protected propertyServicePool:FormControlPropertyServicePool, protected templatePropertiesServiceFactory:TemplatePropertiesServiceFactory) {
    ...
    async createProperties(formControl: FormControl): Promise<BaseProperty<any>[]> {
      ...
      bps.push(await this.propertyServicePool.fieldDefinitionsService().createProperty(formControl.guid,null,formControl.formControlBinding.fieldDefinition.itemSourceDefinition.itemSourceEntityDefinitionId));
      return bps;
    }
    ...
    async loadProperties(formControl: FormControl,uiProperties: BaseProperty<any>[]): Promise<void> {
      ...
      formControl.uiProperties.push(await this.propertyServicePool.fieldDefinitionsService().reloadProperty(
        formControl.guid,
        this.getValue(uiProperties,this.propertyServicePool.fieldDefinitionsService().getName()),
